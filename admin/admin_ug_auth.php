...
if ( isset($HTTP_POST_VARS['submit']) && ( ( $mode == 'user' && $user_id ) || ( $mode == 'group' && $group_id ) ) )
{
	...
	else if ( $mode == 'user' && $HTTP_POST_VARS['userlevel'] == 'modcp' && $user_level != MODCP )
	{

		if ( $userdata['user_id'] != $user_id )
		{
			$sql = "UPDATE " . USERS_TABLE . "
				SET user_level = " . MODCP . "
				WHERE user_id = $user_id";
			if ( !($result = $db->sql_query($sql)) )
			{
				message_die(GENERAL_ERROR, '无法更新用户等级', '', __LINE__, __FILE__, $sql);
			}
			$sql = "DELETE FROM " . AUTH_ACCESS_TABLE . "
				WHERE group_id = $group_id 
					AND auth_mod = 0";
	...
	else
	{
		...
		else if ( $mode == 'user' && $HTTP_POST_VARS['userlevel'] == 'user' && $user_level == MODCP )
		{
		...
		$sql = "SELECT u.user_id 
			...
			WHERE ug.group_id = aa.group_id 
				...
				AND u.user_level NOT IN (" . MOD . ", " . ADMIN . ", " . MODCP . ") 
			GROUP BY u.user_id 
		...
		$sql = "SELECT u.user_id 
			...
			WHERE u.user_level NOT IN (" . USER . ", " . ADMIN . ", " . MODCP . ")
			GROUP BY u.user_id 
			HAVING SUM(aa.auth_mod) = 0";
...
else if ( ( $mode == 'user' && ( isset($HTTP_POST_VARS['username']) || $user_id ) ) || ( $mode == 'group' && $group_id ) )
{
	...
	if ( !($result = $db->sql_query($sql)) )
	{
		message_die(GENERAL_ERROR, '无法得到用户等级信息！', '', __LINE__, __FILE__, $sql);
	}
	$row = $db->sql_fetchrow($result);
	$user_level = $row['user_level'];
	$db->sql_freeresult($result);
	
	$is_modcp = ( $user_level == MODCP) ? true : false;
	...
	while( list($forum_id, $user_ary) = @each($auth_ug) )
	{
		if ( empty($adv) )
		{
			...
			{
				$optionlist_acl = $lang['Not_available'];
			}
		}
		else
		{
			for($j = 0; $j < count($forum_access); $j++)
			{
				if ( $forum_access[$j]['forum_id'] == $forum_id )
				{
					for($k = 0; $k < count($forum_auth_fields); $k++)
					{
						...
						{
							$optionlist_acl_adv[$forum_id][$k] = $lang['Not_available'];
						}
					}
				}
			}
...
//	@reset($auth_user);
	...
	if ( $mode == 'user' )
	{
		...
		if ( $is_admin )
		{
			$s_user_type = '<select name="userlevel"><option value="admin" selected="selected">' . $lang['Auth_Admin'] . '</option><option value="modcp">' . $lang['global_MOD'] . '</option><option value="user">' . $lang['Auth_User'] . '</option></select>';
		}
		else if( $is_modcp)
		{
			$s_user_type = '<select name="userlevel"><option value="admin">' . $lang['Auth_Admin'] . '</option><option value="modcp" selected="selected">' . $lang['global_MOD'] . '</option><option value="user">' . $lang['Auth_User'] . '</option></select>';
		}
		else
		{
			$s_user_type ='<select name="userlevel"><option value="admin">' . $lang['Auth_Admin'] . '</option><option value="modcp">' . $lang['global_MOD'] . '</option><option value="user" selected="selected">' . $lang['Auth_User'] . '</option></select>';
		}
	}
	else
	{
	...
	$template->assign_vars(array(
		...
		'U_UG_SELECT'	 			=> append_sid("admin_ug_auth.$phpEx?mode=group"),
