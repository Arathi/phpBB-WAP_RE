	...
	switch($mode)
	{
		...
		case 'editmedal':
			...
			$template->assign_vars(array(
				...
				"IMAGE_DISPLAY" 		=> ( $medal_image ) ? '<br/><img src="../' . $medal_image . '" />' : "",
				...
				"U_MEDAL_LISTS"			=> append_sid("admin_medal.$phpEx"),
		...
		case 'moderator':
			...
			{
				$select_userlist .= '<input type="checkbox" name="unmod_user[]" value="' . $user_list[$i]['mod_id'] . '" /> ' . $user_list[$i]['username'] . '<br/>';
			...
			if( $select_userlist == '' ) 
			{ 
				$select_userlist = $lang['No_medal_mod'] . '<input type="hidden" name="unmod_user[]" value="-1" />'; 
			}
