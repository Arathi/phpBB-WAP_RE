...
else
{
	...
	if(!$result = $db->sql_query($sql))
	{
		message_die(GENERAL_ERROR, "Could not count smiles", "", __LINE__, __FILE__, $sql);
	}
	$row = $db->sql_fetchrow($result);
	$total_all_smiles = $row['total'];

	$pagination = ( $total_all_smiles > $board_config['topics_per_page'] ) ? generate_pagination("admin_smilies.$phpEx?", $total_all_smiles, $board_config['topics_per_page'], $start) : '';
