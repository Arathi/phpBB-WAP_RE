...
if( !empty($setmodules) )
{
	...
	$module['Users']['Select_Users'] = $filename;
...
if ( $mode == 'edit' || $mode == 'save' && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
{
	...
	if ( ( $mode == 'save' && isset( $HTTP_POST_VARS['submit'] ) ) || isset( $HTTP_POST_VARS['avatargallery'] ) || isset( $HTTP_POST_VARS['submitavatar'] ) || isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		$user_zvanie = ( !empty($HTTP_POST_VARS['user_zvanie']) ) ? trim(strip_tags(htmlspecialchars($HTTP_POST_VARS['user_zvanie']))) : '';
		...
		$signature = ( !empty($HTTP_POST_VARS['signature']) ) ? trim(strip_tags( $HTTP_POST_VARS['signature'] ) ) : '';
	...
	if( isset($HTTP_POST_VARS['avatargallery']) && !$error )
	{
		if( !$error )
		{
			...
			$template->assign_vars(array(
				...
				"U_USERS_ADMIN"			=> append_sid("admin_users.$phpEx"),
	...
	else
	{
		...
		$s_b_year = '<span class="genmed">' . $lang['Year'] . '&nbsp;</span><input type="text" style="width: 50px" name="b_year" size="4" maxlength="4" value="' . $b_year . '" />&nbsp;&nbsp;'; 
		$i = 0;
		$s_birthday = '';
...
{ 
   ...
   default:$gender_no_specify_checked="checked=\"checked\""; 
		...
		$template->assign_vars(array(
			...
			'NIC_COLOR' 						=> $nic_color, 
			'L_USER_ZVANIE' 					=> $lang['User_zvanie'], 
