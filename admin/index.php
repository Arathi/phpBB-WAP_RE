...
if( isset($HTTP_GET_VARS['pane']) && $HTTP_GET_VARS['pane'] == 'left' || isset($HTTP_GET_VARS['pane']) && $HTTP_GET_VARS['pane'] == 'right' )
{
	...
	while( list($cat, $action_array) = each($module) )
	{
		...
		$is_cat = ( isset($HTTP_GET_VARS[POST_CAT_URL]) && !empty($HTTP_GET_VARS[POST_CAT_URL]) ) ? true : false;
		$get_cat = $HTTP_GET_VARS[POST_CAT_URL];
		if ( $is_cat )
		{
			if( $cat == $get_cat )
			{
...
else
{
	...
	$template->assign_vars(array(
		...
		'U_ADMIN_MODS'		=> append_sid("{$phpbb_root_path}mods/admin/index.$phpEx"),
