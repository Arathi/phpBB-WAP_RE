...
if( !empty($setmodules) )
{
	...
	$module['Attachments']['Attach_Manage'] = $filename . '?mode=manage';
...
if ($mode == 'sync')
{
	echo '<div class="navbar"><a href="' . append_sid("{$phpbb_root_path}index.$phpEx") . '">' . $lang['Index'] . '</a>';
	echo '&gt;<a href="' . append_sid("{$phpbb_root_path}admin/index.$phpEx") . '">' . $lang['Admin'] . '</a>';
	echo '&gt;<a href="' . append_sid("{$phpbb_root_path}admin/index.$phpEx?pane=left") . '">' . $lang['Admin_Index_Left'] . '</a>&gt;';
	echo $lang['Syncs'];
	echo '<br />';
	...
	@flush();
	die('<br />'. $lang['Attach_sync_finished'] . '<br /><br />' . $info);
