...
function generate_user_info(&$row, $date_format, $group_mod, &$from, &$posts, &$joined, &$poster_avatar, &$profile_img, &$profile, &$search_img, &$search, &$pm_img, &$pm, &$email_img, &$email, &$www_img, &$www, &$icq_status_img, &$icq_img, &$icq, &$aim_img, &$aim, &$msn_img, &$msn, &$yim_img, &$yim)
{
	...
	if ( !empty($row['user_viewemail']) || $group_mod )
	{
		...
		$email = '/<a href="' . $email_uri . '">e-mail</a>';
	}
	else
	{
		$email = '';
...
else if ( $group_id )
{
	...
	if($userdata['user_id'] == $group_info['group_moderator'] || $userdata['user_level'] == ADMIN) 
	{
		$error = FALSE;
		$error_msg = '';
		include($phpbb_root_path . 'includes/functions_group_logo.'.$phpEx);

		$avatar_sql = '';
		$sendd = append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=$group_id");

		if ( isset($HTTP_POST_VARS['avatardel']) )
		{
			$avatar_sql = "group_logo = ''";
		}
		if ( isset($HTTP_POST_VARS['groupicon']) )
		{
		...
		$delete_image = '<br/><input type="submit" name="avatardel" value="' . $lang['Delete_Image'] . '" />';
	}

	$page_title = $lang['Group_Control_Panel'];
	include($phpbb_root_path . 'includes/page_header.'.$phpEx);

	if ( $group_info['guestbook_enable'] == 1 )
	{
		include($phpbb_root_path . 'includes/functions_guestbook.'.$phpEx);
		if(isset($HTTP_GET_VARS['gb']))
		{
			$gb = new guestbook($group_info, $HTTP_GET_VARS['gb']);
		}
		else
		{
			$gb = new guestbook($group_info, 'view');
		}
	}
...
else
{
	...
	if ( $userdata['session_logged_in'] ) 
	{
		...
		if ( $row = $db->sql_fetchrow($result) )
		{
			...
			do
			{
				...
				if ( $row['user_pending'] )
				{
					$s_pending_groups_opt .= '- <a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=" . $row['group_id'] . "&amp;sid=" . $userdata['session_id']) . '">' . $row['group_name'] . '</a><br/>';
				}
				else
				{
					$s_member_groups_opt .= '- <a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=" . $row['group_id'] . "&amp;sid=" . $userdata['session_id']) . '">' . $row['group_name'] . '</a><br/>';
				}
			}
			while( $row = $db->sql_fetchrow($result) );
	...
	while( $row = $db->sql_fetchrow($result) )
	{
		...
		{
			$s_group_list_opt .= '- <a href="' . append_sid("groupcp.$phpEx?" . POST_GROUPS_URL . "=" . $row['group_id'] . "&amp;sid=" . $userdata['session_id']) . '">' . $row['group_name'] . '</a><br/>';
		}
	}
