...
if ( $mode == 'searchuser' )
{
	if ( isset($HTTP_POST_VARS['search_username']) && !empty($HTTP_POST_VARS['search']) )
	{
		username_search($HTTP_POST_VARS['search_username']);
	}
	elseif ( isset($HTTP_POST_VARS['username_list']) && !empty($HTTP_POST_VARS['use']) )
	{
		$u = intval($HTTP_POST_VARS['username_list']);
		redirect(append_sid("profile.$phpEx?mode=viewprofile&u=$u"));
	}
	else
	{
		username_search('');
...
else if ( $search_keywords != '' || $search_author != '' || $search_id )
{
	...
	if ( $search_id == 'newposts' || $search_id == 'egosearch' || $search_id == 'unanswered' || $search_keywords != '' || $search_author != '' || $search_id == 'bookmarks' )
	{
	...
	if ( $search_results != '' )
	{
		...
		for($i = 0; $i < count($searchset); $i++)
		{
			...
			if ( $show_results == 'bookmarks' )
			{
				$sql_start = "SELECT start
					FROM " . BOOKMARK_TABLE . "
					WHERE user_id = " . $userdata['user_id'] . " 
						AND topic_id = " . $searchset[$i]['topic_id'];
				$result_start = $db->sql_query($sql_start);
				$row_start = $db->sql_fetchrow($result_start);
				$topic_url = append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . '=' . $searchset[$i]['topic_id'] . '&amp;start=' . $row_start['start']);
			}
			else
			{
			...
			else
			{
				...
				$last_post_url = '<a href="' . append_sid("viewtopic.$phpEx?"  . POST_POST_URL . '=' . $searchset[$i]['topic_last_post_id']) . '#' . $searchset[$i]['topic_last_post_id'] . '">' . $lang['View_latest_post'] . '</a>';
