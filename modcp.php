...
if ( $mode == 'reportpost' )
{
	...
	{
		redirect($phpbb_root_path . 'login.'.$phpEx.'?redirect=modcp.'.$phpEx.'&mode=reportpost');
	}
	...
	{
		$mode = ( !empty($HTTP_POST_VARS['action']) ) ? htmlspecialchars($HTTP_POST_VARS['action']) : htmlspecialchars($HTTP_GET_VARS['action']);
	}
	else
	{
		$mode = '';
	...
	if ( ($mode == 'closereport' || $mode == 'openreport') && $report_id != '' )
	{
		...
		if ( empty($last_action_comments) )
		{
			...
			$template->assign_vars(array(
				...
				'S_ACTION'		=> append_sid($phpbb_root_path . 'modcp.'.$phpEx.'?mode=reportpost&amp;action='.$mode.'&amp;report='.$report_id))
			);
		...
		else
		{
			...
			$last_action_user = '<a href="' . append_sid($phpbb_root_path . 'profile.'.$phpEx.'?mode=viewprofile&amp;' . POST_USERS_URL . '=' . $userdata['user_id']) . '">' . $userdata['username'] . '</a>' ;
			...
			{
				$last_action_comments .= '<br/>' . $previous_comments;
			}
			...
			$message =  $lang['Close_success'] . '<br /><br />' . sprintf($lang['Click_return_reports'], '<a href="' . append_sid($phpbb_root_path . 'modcp.'.$phpEx.'?mode=reportpost') . '">', '</a>');
	...
	else if ( $mode == 'close' || $mode == 'open' )
	{
		...
		if ( empty($reports) )
		{
			$message =  $lang['Report_not_selected'] . '<br /><br />' . sprintf($lang['Click_return_reports'], '<a href="' . append_sid($phpbb_root_path . 'modcp.'.$phpEx.'?mode=reportpost') . '">', '</a>');
			message_die(GENERAL_MESSAGE, $message);
		}
		...
		if ( !empty($reports_comments) )
		{
			foreach($reports_comments as $key => $value)
			{
				...
				if ( $previous_comments != '' )
				{
					$last_action_comments = $mass_action_comments . '<br/>' . $previous_comments;
				}
				else
				{
	...
	else if ( $mode == 'optout' || $mode == 'optin' )
	{
		...
		$message = $lang['Opt_success'] . '<br /><br />' . sprintf($lang['Click_return_reports'], '<a href="' . append_sid($phpbb_root_path . 'modcp.'.$phpEx.'?mode=reportpost') . '">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
	}
	else if ( $mode == 'delete' )
	{
		...
		$message =  $lang['Delete_success'] . '<br /><br />' . sprintf($lang['Click_return_reports'], '<a href="' . append_sid($phpbb_root_path . 'modcp.'.$phpEx.'?mode=reportpost') . '">', '</a>');
		message_die(GENERAL_MESSAGE, $message);
	}
	...
	$template->assign_vars(array(
		...
		'U_OPT_OUT' => ( $userdata['user_report_optout'] ) ? append_sid($phpbb_root_path . 'modcp.'.$phpEx.'?mode=reportpost&amp;action=optin') : append_sid($phpbb_root_path . 'modcp.'.$phpEx.'?mode=reportpost&amp;action=optout'),
		...
		'S_ACTION' => append_sid($phpbb_root_path . 'modcp.'.$phpEx.'?mode=reportpost'))
	);
...
if ( !$is_auth['auth_mod'] && !$userdata['user_level'] == MODCP && !( $mode == 'lock' && $topic_poster == $userdata['user_id'] && $userdata['user_id'] != ANONYMOUS ) )
{
	message_die(GENERAL_MESSAGE, $lang['Not_Moderator'], $lang['Not_Authorised']);
}
...
switch( $mode )
{
		...
		if ( $confirm )
		{
			...
			if ($topic_row['forum_postcount'] == 1)
			{
				...
				while ( $row = $db->sql_fetchrow($result) )
				{
					...
					{
						message_die(GENERAL_ERROR, 'Could not update user money count information', '', __LINE__, __FILE__, $sql);
					}
				}
				$db->sql_freeresult($result);
			}
	...
	case 'lock':
		...
		{
			$page_title = $lang['Mod_CP_locktopic'];
			include($phpbb_root_path . 'includes/page_header.'.$phpEx);
		...
		else
		{
			...
			if( ( $total_posts = $db->sql_numrows($result) ) > 0 )
			{
				...
				$template->assign_vars(array(
					...
					'U_MODCP'	=> "modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;start=$start&amp;sid=" . $userdata['session_id'],
				...
				for($i = 0; $i < $total_posts; $i++)
				{
					...
					$poster = '<a href="' . append_sid('profile.'.$phpEx.'?mode=viewprofile&amp;u=' . $poster_id) . '">' . $postrow[$i]['username'] . '</a>';
	...
	default:
		...
		$str = ( isset($HTTP_POST_VARS['str']) ) ? $HTTP_POST_VARS['str'] : $HTTP_GET_VARS['str'];
		if ( $str == 'all' )
		{
			$dob_sql = '';
			$dob_hidden = '<input type="hidden" name="str" value="all" />';
			$dob_url = '<a href="' . append_sid('modcp.'.$phpEx.'?' . POST_FORUM_URL . '=' . $forum_id . '&amp;sid=' . $userdata['session_id']) . '">' . $lang['Paginal_review_topics_modcp'] . '</a>';
		}
		else
		{
			$dob_sql = 'LIMIT ' . $start . ', ' . $board_config['topics_per_page'];
			$dob_hidden = '';
			$dob_url = '<a href="' . append_sid('modcp.'.$phpEx.'?' . POST_FORUM_URL . '=' . $forum_id . '&amp;str=all&amp;sid=' . $userdata['session_id']) . '">' . $lang['All_review_topics_modcp'] . '</a>';
		}
		...
		$sql = "SELECT t.*, u.username, u.user_id, p.post_time
			...
			ORDER BY t.topic_type DESC, p.post_time DESC " . $dob_sql;
		if ( !($result = $db->sql_query($sql)) )
		{
		...
		$template->assign_vars(array(
			'PAGINATION' => ( $str == 'all' ) ? '' : generate_pagination("modcp.$phpEx?" . POST_FORUM_URL . "=$forum_id&amp;sid=" . $userdata['session_id'], $forum_topics, $board_config['topics_per_page'], $start),
