...
for($i = 0; $i < $total_posts; $i++)
{
	...
	if ( $poster_id != ANONYMOUS )
	{
		...
		if ( !empty($postrow[$i]['user_viewemail']) || $is_auth['auth_mod'] )
		{
			...
			$email_img = '<a href="' . $email_uri . '">' . $postrow[$i]['user_email'] . '</a>';
			$email = '<a href="' . $email_uri . '">' . $postrow[$i]['user_email'] . '</a>';
		}
		else
		{
			$email_img = '';
		...
		$www_img = ( $postrow[$i]['user_website'] ) ? '<a href="' . $postrow[$i]['user_website'] . '" target="_userwww">' . $postrow[$i]['user_website'] . '</a>' : '';
		$www = ( $postrow[$i]['user_website'] ) ? '<a href="' . $postrow[$i]['user_website'] . '" target="_userwww">' . $postrow[$i]['user_website'] . '</a>' : '';

		if ( !empty($postrow[$i]['user_icq']) )
		{
			$icq_status_img = '';
			$icq_img = '<small><b>'.$lang['ICQ'].'</b>: <a href="http://wwp.icq.com/scripts/search.dll?to=' . $postrow[$i]['user_icq'] . '">' . $postrow[$i]['user_icq'] . '</a></small>';
			$icq =  '<small><b>'.$lang['ICQ'].'</b>: <a href="http://wwp.icq.com/scripts/search.dll?to=' . $postrow[$i]['user_icq'] . '">' . $postrow[$i]['user_icq'] . '</a></small>';
		}
		else
		{
			$icq_status_img = '';
		...
		{
			$aim_img = ( $postrow[$i]['user_aim'] ) ? '<small><b>'.$lang['AIM'].'</b>: '.$postrow[$i]['user_aim'] . '</small>' : '';
			$aim = ( $postrow[$i]['user_aim'] ) ? '<small><b>'.$lang['AIM'].'</b>: '.$postrow[$i]['user_aim'] . '</small>' : '';
		}
		else
		{
			$aim_img = '';
		...
		if ( !empty($postrow[$i]['user_msn']) )
		{
			$msn_img = ( $postrow[$i]['user_msnm'] ) ? '<small><b>'.$lang['MSNM'].'</b>: ' . $postrow[$i]['user_msnm'] . '</small>' : '';
			$msn = ( $postrow[$i]['user_msnm'] ) ? '<small><b>'.$lang['MSNM'].'</b>: ' . $postrow[$i]['user_msnm'] . '</small>' : '';
		}
		else
		{
			$msn_img = '';
		...
		{
			$yim_img = ( $postrow[$i]['user_yim'] ) ? '<small><b>'.$lang['YIM'].'</b>: ' . $postrow[$i]['user_yim'] . '</small>' : '';
			$yim = ( $postrow[$i]['user_yim'] ) ? '<small><b>'.$lang['YIM'].'</b>: ' . $postrow[$i]['user_yim'] . '</small>' : '';
		}
		else
		{
			$yim_img = '';
