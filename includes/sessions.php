...
function session_userban($user_ip, $user_id)
{
	...
	if ( $ban_info = $db->sql_fetchrow($result) )
	{
		...
		{
			$resban = db_query("SELECT * FROM " . REPUTATION_TABLE . " WHERE (`modification` =4 AND `user_id` =$user_id)");
         			if ( $row = $db->sql_fetchrow($resban) )
			{
             			$moderid = $row['voter_id'];
				$resmoder = db_query("SELECT * FROM " . USERS_TABLE . " WHERE (`user_id` =$moderid)");
				...
				$ban_information = '主持人帐户已被锁定 ' . $modername . ' 不遵守论坛规则！<br />自动恢复仍然存在' . $dni . ' 天（以小时为单位' . $chas . '小时，分钟'. $min . '分钟）<br />同时，有时间，请阅读 <a href="rules.php">规则</a> 并认为自己的行为...';
			}
			else
			{
				$ban_information = '账户被无限期禁止，<br />欲了解更多信息，请联系论坛管理员！';
           			}
		}
	}
	else
