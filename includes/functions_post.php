...
function prepare_message($message, $html_on, $bbcode_on, $smile_on, $bbcode_uid = 0)
{
	...
	$message = preg_replace("#\[rus\](.+?)\[/rus\]#ies","translit('\\1')", $message);
...
function submit_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id, &$topic_type, &$bbcode_on, &$html_on, &$smilies_on, &$attach_sig, &$bbcode_uid, $post_username, $post_subject, $post_message, $poll_title, &$poll_options, &$poll_length, $prop_color, $stick, $highlight)
{
	...
	if ($mode == 'newtopic' || ($mode == 'editpost' && $post_data['first_post']))
	{
		...
		$sql  = ($mode != "editpost") ? "INSERT INTO " . TOPICS_TABLE . " (topic_title, topic_poster, topic_time, forum_id, topic_status, topic_type, topic_vote, topic_color, topic_highlight, topic_stick) VALUES ('$post_subject', " . $userdata['user_id'] . ", $current_time, $forum_id, " . TOPIC_UNLOCKED . ", $topic_type, $topic_vote, '$prop_color', $highlight, $stick)" : "UPDATE " . TOPICS_TABLE . " SET topic_title = '$post_subject', topic_type = $topic_type " . (($post_data['edit_vote'] || !empty($poll_title)) ? ", topic_vote = " . $topic_vote : "") . " WHERE topic_id = $topic_id";
		if (!$db->sql_query($sql))
		{
	...
	$meta = append_sid("viewtopic.$phpEx?" . POST_POST_URL . "=" . $post_id, true); 
$message = $lang['Stored'] . '<br/><a href="' . append_sid("viewtopic.$phpEx?" . POST_POST_URL . "=" . $post_id) . '#' . $post_id . '">&lt;--</a>';
...
function delete_post($mode, &$post_data, &$message, &$meta, &$forum_id, &$topic_id, &$post_id, &$poll_id)
{
	...
	{
		$meta = append_sid("viewforum.$phpEx?" . POST_FORUM_URL . '=' . $forum_id, true);
		$message = $lang['Deleted'];
	}
	else
	{
		$meta = append_sid("viewtopic.$phpEx?" . POST_TOPIC_URL . '=' . $topic_id, true);
...
function user_notification($mode, &$post_data, &$topic_title, &$forum_id, &$topic_id, &$post_id, &$notify_user)
{
	...
	if ($mode != 'delete')
	{
		if ($mode == 'reply')
		{
			...
			$sql = "SELECT u.user_id, u.user_email, u.user_lang, u.user_notify_to_email, u.user_notify_to_pm 
			...
			$notify_pm_subject = $lang['Notify_to_pm_subject'];
			$notify_pm_msg = sprintf($lang['Notify_to_pm_msg'], $topic_title, $topic_url, $topic_stop_watching_url);
					...
					if ( $row['user_notify_to_pm'] )
					{
						...
						if ( $inbox_info = $db->sql_fetchrow($result_pm) )
						{
							if ($board_config['max_inbox_privmsgs'] && $inbox_info['inbox_items'] >= $board_config['max_inbox_privmsgs'])
							{
								...
								if ( !$result_pm = $db->sql_query($sql_pm) )
								{
									message_die(GENERAL_ERROR, 'Could not find oldest privmsgs (inbox)', '', __LINE__, __FILE__, $sql_pm);
								}
								$old_privmsgs_id = $db->sql_fetchrow($result_pm);
								$old_privmsgs_id = $old_privmsgs_id['privmsgs_id'];
				
								$sql_pm = "DELETE $sql_priority FROM " . PRIVMSGS_TABLE . " 
								WHERE privmsgs_id = $old_privmsgs_id";
								if ( !$db->sql_query($sql_pm) )
								{
									message_die(GENERAL_ERROR, 'Could not delete oldest privmsgs (inbox)'.$sql_pm, '', __LINE__, __FILE__, $sql_pm);
								}

								$sql_pm = "DELETE $sql_priority FROM " . PRIVMSGS_TEXT_TABLE . " 
									WHERE privmsgs_text_id = $old_privmsgs_id";
								if ( !$db->sql_query($sql_pm) )
								{
									message_die(GENERAL_ERROR, 'Could not delete oldest privmsgs text (inbox)', '', __LINE__, __FILE__, $sql_pm);
								}
							}
						}

						$sql_info = "INSERT INTO " . PRIVMSGS_TABLE . " (privmsgs_type, privmsgs_subject, privmsgs_from_userid, privmsgs_to_userid, privmsgs_date, privmsgs_ip, privmsgs_enable_html, privmsgs_enable_bbcode, privmsgs_enable_smilies, privmsgs_attach_sig)
							VALUES (" . PRIVMSGS_NEW_MAIL . ", '$notify_pm_subject', -1, " . $to_user_id . ", " . time() . ", '7f000001', 0, 0, 0, 0)";
						...
						$sql_pm = "INSERT INTO " . PRIVMSGS_TEXT_TABLE . " (privmsgs_text_id, privmsgs_bbcode_uid, privmsgs_text)
							VALUES ($privmsg_sent_id, '', '" . mysql_real_escape_string($notify_pm_msg) . "')";
						if ( !$db->sql_query($sql_pm, END_TRANSACTION) )
						{
							message_die(GENERAL_ERROR, "Could not insert/update private message sent text.", "", __LINE__, __FILE__, $sql_pm);
						}
						...
						{
							message_die(GENERAL_ERROR, 'Could not update private message new/read status for user', '', __LINE__, __FILE__, $sql_pm);
						}
					}
