...
if( !empty($setmodules) )
{
	...
	$modcp_module['Users']['Manage'] = $filename;
	return;
}
...
if ( $mode == 'edit' || $mode == 'save' && ( isset($HTTP_POST_VARS['username']) || isset($HTTP_GET_VARS[POST_USERS_URL]) || isset( $HTTP_POST_VARS[POST_USERS_URL]) ) )
{
	...
	else if( !isset( $HTTP_POST_VARS['submit'] ) && $mode != 'save' && !isset( $HTTP_POST_VARS['avatargallery'] ) && !isset( $HTTP_POST_VARS['submitavatar'] ) && !isset( $HTTP_POST_VARS['cancelavatar'] ) )
	{
		...
		if ( $this_userdata['user_level'] == ADMIN || ( $this_userdata['user_level'] == MODCP && $this_userdata['user_id'] != $userdata['user_id']) )
		{
			message_die(GENERAL_MESSAGE, '您没有权限管理该用户！<br />点击 <a href="' . append_sid("admin_users.$phpEx") . '">这里</a> 重新选择用户！');
		}
		//
	...
	else
	{
		...
		// Disable edition for special levels
		$this_user_level_rank = ($this_userdata['user_level'] != COMMITTEE) ? (($this_userdata['user_level'] != MOD) ? $this_userdata['user_level'] : COMMITTEE ) : MOD;
		$user_level_rank = ($userdata['user_level'] != COMMITTEE) ? (($userdata['user_level'] != MOD) ? $userdata['user_level'] : COMMITTEE ) : MOD;

		if (($this_user_level_rank != 0) && ($userdata['user_id'] != $this_userdata['user_id']) && ($this_user_level_rank <= $user_level_rank))
		{
			$template->assign_block_vars('switch_change_disallowed', array());
			$disable_change = ' disabled="disabled"';
		}
		else
		{
...
else
{
	...
	$select_list = '<option value="0" selected="selected">--- ' . $lang['Select_a_User'] . ' ---</option>';
	while ($row = $db->sql_fetchrow($result))
	{
